...
| 기능 | 한국 시간 예약 | 게시 내용 |
| --- | --- | --- |
| 오늘의 질문 | 매일 오전 **9:00** | 미리 준비한 질문 하나와 대화 스레드 |
| 오늘의 할 일 | 월~금 오전 **9:00** | 날짜별 스레드와 할 일/도움 요청 작성 양식 |

평일은 월요일~금요일을 뜻합니다. 공휴일은 별도로 제외하지 않습니다. 실행 환경의 시간대와 관계없이 날짜와 요일은 `Asia/Seoul`을 사용합니다.
...
# 지금 즉시 게시하고 종료: 질문은 매일, 할 일은 평일에만
...
한 기능만 실행하려면 `--job question` 또는 `--job todo`를 붙입니다. `--date`는 미리보기 전용입니다. 실제 실행은 언제나 실행 시작 시점의 한국 날짜로 게시하며 오전 9시를 기다리지 않습니다. 주말에는 수동 실행도 할 일을 건너뜁니다.
...
# .github/workflows/daily-todo.yml — 월~금 한국 시간 09:00
schedule:
  - cron: '0 9 * * 1-5'
    timezone: Asia/Seoul
```
...
- `questions.json`의 질문 40개를 날짜에 따라 순서대로 순환합니다. 목록을 모두 사용한 뒤 다시 반복하며, 목록이 같으면 같은 날짜의 질문도 같습니다. 문자열을 추가/수정할 수 있고 별도 AI 호출은 없습니다.
...
| 할 일이 생성되지 않음 | 한국 날짜가 주말인지, 실행한 `--job`과 Variables 확인 |
