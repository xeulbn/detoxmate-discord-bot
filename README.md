...
| 오늘의 할 일 | 매일 오전 **9:00** | 날짜별 스레드와 한일/병목·오늘 한마디/회고 작성 양식 |
...
한 기능만 실행하려면 `--job question` 또는 `--job todo`를 붙입니다. `--date`는 미리보기 전용입니다. 실제 실행은 언제나 실행 시작 시점의 한국 날짜로 게시하며 오전 9시를 기다리지 않습니다. 양식의 월/일과 한글 요일도 해당 날짜에 맞춰 자동으로 바뀝니다.
